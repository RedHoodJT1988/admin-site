...
const columnDefsMapper = (field: Field): ColDef => ({
  ...field,
  field: field.name,
...
function App() {
  ...
  const getData = async () => {
    ...
    try {
      res = await (await fetch("/api/sales", { method: "POST", body, headers })).json();
      setRawData(res.data);
  ...
  return (
    ...
    <select value={dataset} onChange={(e) => setDataset(e.target.value as Dataset)}>
      ...
      />
      <button onClick={dataset === "sales" ? getData : getDataUNL}>Get Data</button>
      <QueryBuilder
        fields={fields[dataset]}
